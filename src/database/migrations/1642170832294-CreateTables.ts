...
export class CreateTables1642170832294 implements MigrationInterface {
    name = 'CreateTables1642170832294'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int NOT NULL AUTO_INCREMENT, \`code\` varchar(255) NOT NULL, \`total\` decimal(10,2) NOT NULL, \`note\` mediumtext NULL, \`status\` enum ('na fila', 'pronto', 'em transito', 'entregue', 'cancelado') NOT NULL DEFAULT 'na fila', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`addressId\` int NOT NULL, \`clientId\` int NOT NULL, UNIQUE INDEX \`REL_73f9a47e41912876446d047d01\` (\`addressId\`), UNIQUE INDEX \`REL_9b27855a9c2ade186e5c55d1ec\` (\`clientId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`REL_9b27855a9c2ade186e5c55d1ec\` ON \`order\``);
